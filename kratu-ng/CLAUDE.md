...
speaks Hausa; the child answers by voice or by touch. **This Angular project is THE app** (Sani, 13 Sep 2026). The old
single-file build (`../tools/build_v2.py`, `../v2/`, `../kratu-v2.html`) and the design page (`../kratu-login.artifact.html`)
are retired: read them for history, never extend them.
...
- Parity with the old build is measured, not assumed: `scripts/parity/` (collector + compare) — see its README. Run it
  after any layout change. The old build reports rects in layout px (legacy CSS zoom); the Angular page in screen px.
...
## Open items
- Review page inside the app (`/review`, debug only), screens index (`/screens`), moving this project to the repo root
  with the old folders archived, the Pages deploy workflow — see the plan: https://claude.ai/code/artifact/56d76491-ae9f-476e-b17d-895b578e5300
