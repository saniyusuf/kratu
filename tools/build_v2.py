...
#loader{position:fixed;inset:0;z-index:250;background:#FBF3DE;background-image:radial-gradient(rgba(150,120,70,.10) 1.6px,transparent 1.6px);background-size:20px 20px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;font-family:var(--disp,sans-serif);color:#3B2E1E;text-align:center;padding:24px;transition:opacity .45s}
#loader.out{opacity:0;pointer-events:none}
#loader .ld-laila{width:clamp(90px,17vh,150px);height:clamp(90px,17vh,150px)}
#loader .ld-word{display:flex;gap:.04em;font-weight:900;font-size:clamp(44px,12vh,92px);line-height:1;-webkit-user-select:none;user-select:none}
#loader .ld-word span{color:#DCD0B4;display:inline-block;transition:color .3s}
#loader .ld-word span.on{animation:ldpop .55s cubic-bezier(.34,1.56,.64,1)}
#loader .ld-word .k.on{color:var(--red)}#loader .ld-word .r.on{color:var(--orange)}#loader .ld-word .a.on{color:var(--yellow)}#loader .ld-word .t.on{color:var(--teal)}#loader .ld-word .u.on{color:var(--blue)}
@keyframes ldpop{0%{transform:scale(.55) rotate(-10deg)}60%{transform:scale(1.2) rotate(5deg)}100%{transform:scale(1) rotate(0)}}
#loader .ld-bar{width:min(440px,72vw);height:14px;border-radius:99px;background:#EADFC3;overflow:hidden;box-shadow:inset 0 1px 2px rgba(0,0,0,.08)}
#loader .ld-bar i{display:block;height:100%;width:0;border-radius:99px;background:linear-gradient(90deg,var(--red),var(--orange),var(--yellow),var(--teal),var(--blue));transition:width .6s}
#loader .ld-msg b{display:block;font-size:clamp(18px,3.6vh,26px)}#loader .ld-msg small{display:block;margin-top:4px;font:600 clamp(13px,2.4vh,16px) var(--body,sans-serif);color:#8a7a5c;max-width:40ch}
#loader .ld-steps{display:flex;gap:8px;flex-wrap:wrap;justify-content:center;font:700 12px var(--body,sans-serif);color:#8a7a5c}
#loader .ld-steps span{padding:4px 11px;border-radius:99px;background:#F1E7CF}#loader .ld-steps span.busy{animation:ldpulse 1.1s ease-in-out infinite}#loader .ld-steps span.done{background:#DDF3EE;color:#1E7F73}#loader .ld-steps span.fail{background:#FBE2E0;color:#B3261E}
@keyframes ldpulse{0%,100%{opacity:1}50%{opacity:.45}}
#loader .ld-retry,#loader .ld-skip{display:none;border:0;border-radius:14px;padding:12px 22px;font:800 17px var(--body,sans-serif);cursor:pointer}
#loader .ld-retry{background:#2FAE9E;color:#fff;box-shadow:0 3px 0 #1E8A7C}#loader .ld-retry:active{transform:translateY(2px);box-shadow:0 1px 0 #1E8A7C}
#loader .ld-skip{background:transparent;color:#8a7a5c;text-decoration:underline;font-size:14px;padding:6px}
#loader.failed .ld-retry{display:inline-block}#loader.failed .ld-skip:not([hidden]){display:inline-block}
...
(function(){
  ...
  (function(){ var gear=document.getElementById('admingear'), panel=document.getElementById('admin'); var NAMES=null;
    ...
    function nameClip(key,cb){ if(!NAMES&&window.KRATU_NAMES_AUDIO) NAMES=window.KRATU_NAMES_AUDIO; if(NAMES){ cb(NAMES[key]||null); return; } fetch('names_audio.json').then(function(r){ return r.json(); }).then(function(j){ NAMES=j||{}; cb(NAMES[key]||null); }).catch(function(){ NAMES={}; cb(null); }); }
  ...
  /* boot loader: every heavy piece (speech model, face engine, name bank, name audio, fonts and pictures) is fetched before the first screen.
     The Kratu word fills in letter by letter as the pieces arrive; a piece that fails can be retried, and after two failed rounds the adult may continue anyway (Sani 2026-09-12) */
  var LOADER=(function(){
    var el=document.getElementById('loader'); if(!el) return Promise.resolve();
    var word=el.querySelectorAll('.ld-word span'), bar=el.querySelector('.ld-bar i'), msg=el.querySelector('.ld-msg b'), sub=el.querySelector('.ld-msg small'), steps=el.querySelector('.ld-steps'), retry=el.querySelector('.ld-retry'), skip=el.querySelector('.ld-skip');
    try{ var tpl=document.getElementById('lailaTpl'); var lv=tpl?tpl.content.querySelector('svg'):document.querySelector('svg.laila-v'); if(lv) el.querySelector('.ld-laila').appendChild(lv.cloneNode(true)); }catch(e){}
    var FAILTEST=(new URLSearchParams(location.search).get('ldfail')||'').split(',').filter(Boolean);   /* ?ldfail=bank,speech makes those pieces fail on their first attempt, to see the retry */
    function hasTag(src){ return !!document.querySelector('script[src="'+src+'"]'); }
    function reinject(src){ return new Promise(function(res,rej){ var t=document.createElement('script'); t.src=src+'?r='+Date.now(); t.onload=function(){ res(); }; t.onerror=function(){ rej(new Error(src+' not reachable')); }; document.head.appendChild(t); }); }
    function poll(test,ms,tmo){ return new Promise(function(res,rej){ var t0=Date.now(); (function w(){ var r=test(); if(r==='ok') return res(); if(r==='fail') return rej(new Error('failed')); if(Date.now()-t0>tmo) return rej(new Error('timeout')); setTimeout(w,ms); })(); }); }
    function isJSON(r){ return /json/i.test(r.headers.get('content-type')||''); }
    function download(url,onp){ return fetch(url).then(function(r){ if(!r.ok) throw new Error(url+' '+r.status); var total=+r.headers.get('content-length')||0, got=0; if(!r.body||!r.body.getReader){ return r.arrayBuffer().then(function(){}); } var rd=r.body.getReader(); return new Promise(function(res,rej){ (function pump(){ rd.read().then(function(x){ if(x.done){ res(); return; } got+=x.value.length; onp(got,total); pump(); }).catch(rej); })(); }); }); }   /* the model is fetched once here, with a byte count, and the recogniser's own worker then finds it in the cache */
    var errs={}; function prog(id,text){ var i=ST.map(function(st){ return st.id; }).indexOf(id); if(i<0) return; var st=ST[i]; steps.children[i].textContent=st.ha+' · '+st.en+(text?' · '+text:''); }
    (function(){ var said=false; function say(){ if(said) return; said=true; try{ var a=new Audio(KRATU_AUDIO.s_loading); a.play().catch(function(){ said=false; }); }catch(e){ said=false; } } setTimeout(say,150); el.addEventListener('pointerdown', function(){ say(); }, true); })();   /* Laila: "please wait a little, everything is getting ready" — at once where the tablet allows sound before a tap, otherwise on the first touch */
    function getJSON(url){ return fetch(url).then(function(r){ if(r.status===404||!isJSON(r)) return null; if(!r.ok) throw new Error(url+' '+r.status); return r.json(); }); }
    try{ if(!window.__kratuBankP) window.__kratuBankP=fetch('names_bank_expanded.json'); }catch(e){}   /* shared with the design's own name-bank fetch: one download */
    var ST=[
      {id:'speech', ha:'Murya', en:'voice', run:function(){ if(!hasTag('vosk.js')) return Promise.resolve('skip'); var pre=(typeof Vosk==='undefined')?reinject('vosk.js'):Promise.resolve(); var kicked=false;
        return pre.then(function(){ return download('vosk-model.tar.gz', function(got,total){ prog('speech', total?(Math.round(got/1048576)+' / '+Math.round(total/1048576)+' MB'):(Math.round(got/1048576)+' MB')); }); }).then(function(){ prog('speech','ana buɗewa · unpacking'); return poll(function(){ var sp=window.kratuSpeech; if(!sp) return 'wait'; if(sp.ready()) return 'ok'; var st=sp.state(); if(st==='failed'){ if(kicked) return 'fail'; kicked=true; sp.retry(); return 'wait'; } if(st==='idle') sp.loadModel(); return 'wait'; },250,300000); }); }},
      {id:'face', ha:'Fuska', en:'face', run:function(){ if(!hasTag('kratu-id.js')) return Promise.resolve('skip'); var pre=(!window.KratuID)?reinject('kratu-id.js'):Promise.resolve();
        return pre.then(function(){ if(!window.KratuID) throw new Error('no engine'); try{ if(window.faceapi&&!window.KRATU_FACE){ window.KRATU_FACE=Promise.all([faceapi.nets.tinyFaceDetector.loadFromUri('models/faceapi'),faceapi.nets.faceLandmark68TinyNet.loadFromUri('models/faceapi'),faceapi.nets.faceRecognitionNet.loadFromUri('models/faceapi')]); window.KRATU_FACE.catch(function(){ window.KRATU_FACE=null; }); } }catch(e){}   /* the fallback detector warms up alongside, never blocks */
          return Promise.race([ window.KratuID.init().then(function(){ return window.KratuID.store.load(); }), new Promise(function(_,rej){ setTimeout(function(){ rej(new Error('timeout')); },300000); }) ]); }); }},
      {id:'bank', ha:'Sunaye', en:'names', run:function(){ if(window.KRATU_BANK) return Promise.resolve(); var first=window.__kratuBankP; window.__kratuBankP=null;
        if(first) return first.then(function(r){ if(r.status===404||!isJSON(r)) return 'skip'; if(!r.ok) throw new Error('bank '+r.status); return poll(function(){ return window.KRATU_BANK?'ok':'wait'; },150,60000); });
        return getJSON('names_bank_expanded.json').then(function(j){ if(j===null) return 'skip'; if(window.__kratuSetBank) window.__kratuSetBank(j); else window.KRATU_BANK=j; }); }},
      {id:'audio', ha:'Muryoyin sunaye', en:'name audio', run:function(){ if(window.KRATU_NAMES_AUDIO) return Promise.resolve(); return getJSON('names_audio.json').then(function(j){ if(j===null) return 'skip'; window.KRATU_NAMES_AUDIO=j; }); }},
      {id:'images', ha:'Hotuna', en:'pictures', run:function(){ var fonts=(document.fonts&&document.fonts.ready)?document.fonts.ready:Promise.resolve(); var imgs=[]; try{ var W=window.KRATU_WORDS||{}; Object.keys(W).slice(0,4).forEach(function(c){ (W[c]||[]).slice(0,2).forEach(function(w){ if(w.img) imgs.push(new Promise(function(res){ var im=new Image(); im.onload=im.onerror=function(){ res(); }; im.src=w.img; })); }); }); }catch(e){} return Promise.all([fonts].concat(imgs)).then(function(){}); }}
    ];
    var state={}, fails=0, resolveAll, all=new Promise(function(r){ resolveAll=r; });
    ST.forEach(function(st){ var c=document.createElement('span'); c.textContent=st.ha+' · '+st.en; steps.appendChild(c); });
    function paint(){ var done=0; ST.forEach(function(st,i){ var v=state[st.id]||'wait'; if(v==='ok'||v==='skip'){ word[i].classList.add('on'); done++; } steps.children[i].className=(v==='ok'||v==='skip')?'done':(v==='fail'?'fail':(v==='busy'?'busy':'')); }); bar.style.width=Math.round(done/ST.length*100)+'%'; if(done===ST.length){ msg.textContent='An shirya!'; sub.textContent='Ready'; } }
    function runOne(st){ state[st.id]='busy'; paint(); var p; try{ p=Promise.resolve(st.run()); }catch(e){ p=Promise.reject(e); } if(FAILTEST.indexOf(st.id)>=0){ FAILTEST.splice(FAILTEST.indexOf(st.id),1); p=p.then(function(){ throw new Error('test failure'); }); }
      return p.then(function(r){ state[st.id]=(r==='skip')?'skip':'ok'; prog(st.id,''); paint(); }).catch(function(e){ state[st.id]='fail'; errs[st.id]=(e&&e.message)||String(e); prog(st.id,''); try{ console.warn('Kratu loader:',st.id,errs[st.id]); }catch(_){} paint(); }); }
    function finish(){ el.classList.add('out'); setTimeout(function(){ try{ el.remove(); }catch(e){} },500); resolveAll(); }
    function run(){ el.classList.remove('failed'); msg.textContent='Ana shirya Kratu…'; sub.textContent='Getting ready · after this it works without internet'; var todo=ST.filter(function(st){ return state[st.id]!=='ok'&&state[st.id]!=='skip'; });
      Promise.all(todo.map(runOne)).then(function(){ var bad=ST.filter(function(st){ return state[st.id]==='fail'; }); if(!bad.length){ setTimeout(finish,600); return; } fails++; el.classList.add('failed'); msg.textContent='Wani abu bai zo ba'; sub.textContent='Something did not load: '+bad.map(function(st){ return st.en+' ('+(errs[st.id]||'?')+')'; }).join(', ')+' · check the connection, then try again'; if(fails>=2) skip.hidden=false; }); }
    retry.addEventListener('click', run); skip.addEventListener('click', finish);
    window.kratuLoader={state:function(){ return JSON.parse(JSON.stringify(state)); }, retry:run};
    run(); return all;
  })();
...
app=(tpl+'<div id="app"><div class="tabs"><button id="tb-working"></button><button id="tb-agreed"></button><button id="tb-lab"></button></div>'
     ...
     '<div id="loader"><div class="ld-laila"></div><div class="ld-word"><span class="k">K</span><span class="r">r</span><span class="a">a</span><span class="t">t</span><span class="u">u</span></div><div class="ld-bar"><i></i></div><div class="ld-msg"><b>Ana shirya Kratu…</b><small>Getting ready · after this it works without internet</small></div><div class="ld-steps"></div><button class="ld-retry">Sake gwadawa · Try again</button><button class="ld-skip" hidden>Ci gaba haka · Continue anyway</button></div>'
...
scripts=scripts.replace(' var BANK=null, BANKIDX=null;',' var BANK=null, BANKIDX=null; window.__kratuSetBank=function(j){ BANK=j; window.KRATU_BANK=j; BANKIDX=null; };',1)
scripts=scripts.replace("try{ fetch('names_bank_expanded.json')","try{ (window.__kratuBankP||fetch('names_bank_expanded.json'))",1)
assert '__kratuSetBank' in scripts and '__kratuBankP' in scripts
